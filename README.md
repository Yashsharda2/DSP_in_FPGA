...
## FFT
The FFT stage consists of calculating the Discrete Fourier Transform of a sequence of 16 filtered samples using the Radix-2 Decimation in Frequency algorithm. A naive DFT would require O(N²) multiplications, but the Radix-2 FFT reduces the number of multiplications to O(N log N) using a divide-and-conquer approach to recursively divide the DFT into smaller butterfly operations. Each butterfly takes two complex numbers as input, applies a twiddle factor, and produces two complex output numbers. The FFT has 4 stages of butterflies to calculate the 16-point DFT, with twiddle factors precomputed and stored in Q1.15 format.
